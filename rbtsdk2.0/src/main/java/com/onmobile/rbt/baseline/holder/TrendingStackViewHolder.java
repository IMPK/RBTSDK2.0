...
public class TrendingStackViewHolder extends StackViewHolder<List<RingBackToneDTO>> {
    ...
    public TrendingStackViewHolder(Context context, View root, View loading, View content, FragmentManager fragmentManager) {
    ...
    protected void initViews() {
        mAppLocaleHelper=AppManager.getAppLocaleManager(AppManager.getContext());
        if (contentLayout != null) {
            viewPager = contentLayout.findViewById(R.id.viewpager_trending_stack_discover);
            pagerIndicator = contentLayout.findViewById(R.id.indicator_trending_stack_discover);
    ...
    public void bindHolder(List<RingBackToneDTO> ringBackToneDTOS) {
        ...
        if (ringBackToneDTOS != null && ringBackToneDTOS.size() > 0) {
            ...
            if (sliderAdapter.getCount() > 0) {
                ...
                viewPager.setCurrentItem(currentItemCount);
                viewPager.post(() -> {
    ...
    private OnItemClickListener<RingBackToneDTO> mItemClickListener = new OnItemClickListener<RingBackToneDTO>() {
        ...
        public final void onItemClick(View view, RingBackToneDTO ringBackToneDTO, int position, Pair<View, String>... sharedElements) {
            if(view.getId() == R.id.ib_play_trending_stack_discover) {
                ...
                WidgetUtils.getSetCallerTuneBottomSheet(AnalyticsConstants.EVENT_PV_SOURCE_SET_FROM_TRENDING_CARD, ringBackToneDTO).setCallback(new OnBottomSheetChangeListener() {
                    @Override
    ...
    private BaselineMusicPlayer.MusicPreviewListener mMusicPreviewListener = new BaselineMusicPlayer.MusicPreviewListener() {
        ...
        public void onPreviewPlaying(int progressStatus) {
            ...
            if (!isUIVisible || mBottomSheetOpened)
                stopMusic(false);
            if (mBaselineMusicPlayer != null && AppUtils.isRecommendationQueueDelayLapsed(mBaselineMusicPlayer.getMediaPlayedInSeconds())) {
    ...
    private void register(Context context) {
        if (context != null) {
            ...
            LocalBroadcastManager.getInstance(context).registerReceiver(getVisibilityChangeBroadcastReceiver(), new IntentFilter(AppConstant.KEY_BROADCAST_ACTION_VISIBILITY_CHANGE_TRENDING_STACK));
        }
    }
