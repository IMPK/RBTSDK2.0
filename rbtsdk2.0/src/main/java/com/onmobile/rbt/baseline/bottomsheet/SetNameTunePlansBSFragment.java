...
public class SetNameTunePlansBSFragment extends BaseFragment {
    ...
    private AppCompatTextView mTvArtistPreview, mTvTrackPreview, mTvSetTune, mTvAddMoreContactCount;
    ...
    public static SetNameTunePlansBSFragment newInstance(String callerSource, RingBackToneDTO ringBackToneDTO) {
        SetNameTunePlansBSFragment fragment = new SetNameTunePlansBSFragment();
    ...
    protected String initTag() {
        return SetNameTunePlansBSFragment.class.getSimpleName();
    }
    ...
    public void loadView() {
        ...
        isSelected = BaselineApplication.getApplication().getRbtConnector().isRingbackSelected(mRingBackToneDTO.getId());
        if (isSelected) {
        ...
        mTvArtistPreview.setText(mRingBackToneDTO.getPrimaryArtistName());
    ...
    public SetNameTunePlansBSFragment setCallback(BottomSheetFragmentListener<BaseFragment, RingBackToneDTO> callback) {
            ...
            public void success(RingBackToneDTO result) {
                ...
                if (pricingSubscriptionDTOS != null && pricingSubscriptionDTOS.size() > 0) {
                    ...
                    for (PricingSubscriptionDTO priceDTO : pricingSubscriptionDTOS) {
                        ...
                        if (priceDTO.getRetail_priceObject() != null &&
    ...
    private void updatePeekHeight() {
        if (mCallback != null)
            mCallback.updatePeekHeight(SetNameTunePlansBSFragment.this);
    }
...
//                    }
//                    if (mCallback != null)
//                        mCallback.next(SetNameTunePlansBSFragment.this, mRingBackToneDTO);
//                } else {
//                    if (mCallback != null)
//                        mCallback.done(SetNameTunePlansBSFragment.this, mRingBackToneDTO);
//                }
...
//                            mCallback.done(SetCallerTunePlansBSFragment.this, mRingBackToneDTO);
    ...
    private void setTune(AppBaselineCallback<PurchaseComboResponseDTO> callback) {
        ...
        BaselineApplication.getApplication().getRbtConnector().getAppUtilityNetworkRequest(mCallerSource, ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, contactMap, new AppBaselineCallback<AppUtilityDTO>() {
            ...
            public void success(AppUtilityDTO result) {
                ...
                boolean isAllowed = BaselineApplication.getApplication().getRbtConnector().isThirdPartyPaymentRequired(ringBackToneDTO.getId(), pricingSubscriptionDTO, pricingIndividualDTO);

                if (mPlanLayout.getSelectedPlan() != null && mPlanLayout.getSelectedPlan().isGift()) {
                    purchaseGift(ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, contactMap, null, mPlanLayout.getSelectedPlan().getParentRefId(), callback);
                    ...
                    purchase(ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, contactMap, null, callback);
                }
    ...
    private void purchase(RingBackToneDTO ringBackToneDTO, PricingSubscriptionDTO pricingSubscriptionDTO, PricingIndividualDTO pricingIndividualDTO, Map<String, String> contactMap, Map<String, String> extraInfoMap, AppBaselineCallback<PurchaseComboResponseDTO> callback) {
        ...
        if (!isShowConsentPopUp(confirmationType, mIsUpgrade)) {
            //sendConsentAnalytics(AnalyticsConstants.EVENT_PV_USER_CONSENT_TYPE_INLINE, AnalyticsConstants.EVENT_PV_USER_CONSENT_RESULT_YES);
            BaselineApplication.getApplication().getRbtConnector().purchase(ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, contactMap, extraInfoMap, callback);
        } else {
            showProgress(false);
        ...
        /*BaselineApplication.getApplication().getRbtConnector().getAppUtilityNetworkRequest(mCallerSource, ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, contactMap, new AppBaselineCallback<AppUtilityDTO>() {
            ...
            public void success(AppUtilityDTO result) {
                ...
                BaselineApplication.getApplication().getRbtConnector().setAppUtilityDTO(result);
                BaselineApplication.getApplication().getRbtConnector().dummyPurchase(ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, contactMap,null);
                mNetworkType = result.getNetworkType();
    ...
    private void purchaseGift(RingBackToneDTO ringBackToneDTO, PricingSubscriptionDTO pricingSubscriptionDTO, PricingIndividualDTO pricingIndividualDTO, Map<String, String> contactMap, Map<String, String> extraInfoMap, String parentRefId, AppBaselineCallback<PurchaseComboResponseDTO> callback) {
        ...
        if (!isShowConsentPopUp(confirmationType, mIsUpgrade)) {
            BaselineApplication.getApplication().getRbtConnector().purchaseGift(ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, contactMap, extraInfoMap, mPlanLayout.getSelectedPlan().getParentRefId(), callback);
        } else {
            showProgress(false);
            AppDialog.PurchaseConfirmDialogFragment(getRootActivity(), mConfirmationPopUpDescription, new PurchaseConfirmDialog.ActionCallBack() {
                @Override
        ...
        builder.setMessage(getString(R.string.gift_suggestion_popup_message))
                ...
                .setPositiveButton(getString(R.string.gift_suggestion_popup_continue), new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        //((HomeActivity)getRootActivity()).moveToProfile();
                        if (getActivity() instanceof HomeActivity) {
                            ((HomeActivity) getActivity()).moveToProfile();
                        } else {
                            SetCallerTunePlansBSFragment.fromPreBuyAudioShowPopUp = true;
                        }

                    }
                })
                .setNegativeButton(getString(R.string.gift_suggestion_popup_cancel), new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                        dialog.cancel();
                    }
                });
    ...
    private void purchaseOperator(Map<String, String> extraInfoMap) {
        ...
        if (mPlanLayout.getSelectedPlan() != null && mPlanLayout.getSelectedPlan().isGift()) {
            purchaseGift(ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, mSelectedContacts, extraInfoMap, mPlanLayout.getSelectedPlan().getParentRefId(), callbackPayTM);
        } else {
            purchase(ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, mSelectedContacts, extraInfoMap, callbackPayTM);
        }
    }
    ...
    private void purchasePayTM(Map<String, String> extraInfoMap) {
        ...
        if (mPlanLayout.getSelectedPlan() != null && mPlanLayout.getSelectedPlan().isGift()) {
            ...
            BaselineApplication.getApplication().getRbtConnector().purchaseGift(ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, mSelectedContacts, extraInfoMap, mPlanLayout.getSelectedPlan().getParentRefId(), callbackPayTM);
            ...
            BaselineApplication.getApplication().getRbtConnector().purchase(ringBackToneDTO, pricingSubscriptionDTO, pricingIndividualDTO, mSelectedContacts, extraInfoMap, callbackPayTM);
