...
public class UpdateUserShufflePlaylistRequest extends BaseAPIStoreRequestAction {
    ...
    private String udp_id, name, extra_info;
    private Call<UserDefinedPlaylistDTO> call;

    public UpdateUserShufflePlaylistRequest(String udp, String name, String extra_info, BaselineCallback<UserDefinedPlaylistDTO> dtoBaselineCallback) {
    ...
    public void initCall() {
        call = getApi().updateUserDefinedPlaylist(getBaseStoreURLRequest(),udp_id, getCreateUDPOptions(), getParams());
    }
    ...
    private CreateUserDefinedPlaylistQueryParams getParams() {
        ...
        if (name != null){ params.setName(name);}
        if(extra_info!=null) {
            params.setExtra_info(extra_info);
        }
        return params.build();
    }
