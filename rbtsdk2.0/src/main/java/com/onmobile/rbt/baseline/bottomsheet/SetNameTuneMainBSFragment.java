...
public class SetNameTuneMainBSFragment extends BaseBottomSheetFragment<SetNameTuneMainBSFragment> implements BottomSheetFragmentListener<BaseFragment, RingBackToneDTO> {
    ...
    public static SetNameTuneMainBSFragment newInstance(String callerSource, RingBackToneDTO ringBackToneDTO) {
        SetNameTuneMainBSFragment fragment = new SetNameTuneMainBSFragment();
    ...
    protected String initTag() {
        return SetNameTuneMainBSFragment.class.getSimpleName();
    }

    @Override
    protected int initLayout() {
        return R.layout.fragment_set_name_tune_main_bs;
    }
    ...
    protected void bindViews(View view) {
        ...
        addInitialFragment(
                mRegistrationRequired ?
                        ...
                        SetNameTunePlansBSFragment.newInstance(mCallerSource, mRingBackToneDTO).setCallback(this)
        );
    }
    ...
    @Override
    public SetNameTuneMainBSFragment setCallback(OnBottomSheetChangeListener callback) {
    ...
    public void next(BaseFragment fragment, RingBackToneDTO data) {
        ...
        if (fragment instanceof RegistrationBSFragment) {
            nextFragment = SetNameTunePlansBSFragment.newInstance(mCallerSource, mRingBackToneDTO).setCallback(this);
            sheetCancellable = true;
        } else if (fragment instanceof SetNameTunePlansBSFragment) {
            hideTitle();
    ...
    public void done(BaseFragment fragment, RingBackToneDTO data) {
        if (!isAdded()) return;
        if (fragment instanceof SetNameTunePlansBSFragment) {
