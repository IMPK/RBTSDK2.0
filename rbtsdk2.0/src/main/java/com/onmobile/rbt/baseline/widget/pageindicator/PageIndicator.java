package com.onmobile.rbt.baseline.widget.pageindicator;
...
/**
 * Created by Shahbaz Akhtar on 22/03/2019.
 *
...
public class PageIndicator extends View {
    ...
    public PageIndicator(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray attributes = context.obtainStyledAttributes(
                attrs, R.styleable.PagerIndicator, defStyleAttr, R.style.PagerIndicator);
        dotColor = attributes.getColor(R.styleable.PagerIndicator_page_indicator_dotColor, 0);
        selectedDotColor = attributes.getColor(R.styleable.PagerIndicator_page_indicator_dotSelectedColor, dotColor);
        dotNormalSize = attributes.getDimensionPixelSize(R.styleable.PagerIndicator_page_indicator_dotSize, 0);
        dotSelectedSize = attributes.getDimensionPixelSize(R.styleable.PagerIndicator_page_indicator_dotSelectedSize, 0);
        spaceBetweenDotCenters = attributes.getDimensionPixelSize(R.styleable.PagerIndicator_page_indicator_dotSpacing, 0) + dotNormalSize;
        looped = attributes.getBoolean(R.styleable.PagerIndicator_page_indicator_looped, false);
        int visibleDotCount = attributes.getInt(R.styleable.PagerIndicator_page_indicator_visibleDotCount, 0);
        setVisibleDotCount(visibleDotCount);
        visibleDotThreshold = attributes.getInt(R.styleable.PagerIndicator_page_indicator_visibleDotThreshold, 2);
        attributes.recycle();
    ...
    public void attachToPager(@NonNull ViewPager pager) {
        attachToPager(pager, new ViewPagerPageAttacher());
    }
    ...
    public void attachToRecyclerView(@NonNull RecyclerView recyclerView) {
        attachToPager(recyclerView, new RecyclerViewPageAttacher());
    }
    ...
    public void attachToRecyclerView(@NonNull RecyclerView recyclerView, int currentPageLeftCornerX) {
        attachToPager(recyclerView, new RecyclerViewPageAttacher(currentPageLeftCornerX));
    }
    ...
    public interface PagerAttacher<T> {
         ...
         * You must call:
         * {@link PageIndicator#setDotCount(int)} - initially and after page selection,
         * {@link PageIndicator#setCurrentPosition(int)} - initially and after page selection,
         * {@link PageIndicator#onPageScrolled(int, float)} - in your pager callback to track scroll offset,
         * {@link PageIndicator#reattach()} - each time your adapter items change.
         *
         * @param indicator indicator
         * @param pager     pager to attach
         */
        void attachToPager(@NonNull PageIndicator indicator, @NonNull T pager);
